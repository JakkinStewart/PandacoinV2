...
#if defined(NDEBUG)
# error "PandacoinV2 cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x45222ffaad9c9648da6e073292d7049060efe3d3acbf1acf677106aa0016ab38");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PandacoinV2: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PandacoinV2 Signed Message:\n";
...
{
    // PandacoinV2: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // PandacoinV2: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // PandacoinV2: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // PandacoinV2: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PandacoinV2: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PandacoinV2: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x45222ffaad9c9648da6e073292d7049060efe3d3acbf1acf677106aa0016ab38");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Breaking News: No one cares";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x681ba0d9c0784dc18abdc3fe4fe565a2f79ca749afc4d253287147cebc9915ac"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PandacoinV2: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PandacoinV2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PandacoinV2Miner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PandacoinV2Miner(CWallet *pwallet)
{
    printf("PandacoinV2Miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pandacoinv2-miner");
    ...
    try { loop {
        ...
        printf("Running PandacoinV2Miner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PandacoinV2Miner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PandacoinV2Miner, pwallet));
}
